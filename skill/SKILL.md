...
касания; полнотекст codbash — запасная.
...
- Текущую сессию игнорируй. Если индекс молчит — fallback: `codbash search "<запрос>"`
  (если установлен), затем шаг 2 по найденному id.
...
- последним вариантом всегда — **«Копнуть глубже»**: по выбору открой карточки
  (`recall show`) всех кандидатов, при нужде сырые transcripts или
  `codbash search`, и покажи расширенное сравнение.
...
3. **spawn — открыть окно самому.** «Открой её», «переключи меня», «дай окошко»:
   ...
   (iTerm: `RECALL_TERMINAL=iTerm recall open <id8>`). Это живой контекст той сессии —
   пользователь продолжает там, ты в своём окне свободен.
